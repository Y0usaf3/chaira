...
fn make_random_record(table_id: &TableId, rng: &mut impl RngExt) -> Result<InsertRecord, String> {
    ...
    cells.insert(
        "Score".into(),
        CellValue::new(Value::Decimal(
            DecimalValue::new(Some(rng.random::<f64>() * 100.0), None)
                .map_err(|e| format!("{e}"))?,
        )),
    );
    cells.insert(
...
async fn burst_worker(worker_id: usize) -> Result<(), String> {
    ...
    let (token, _) = timed!(
        STATS.create_session,
        SessionService::create_session(
            models::InsertSession {
                ip: "10.0.0.1".into(),
                user_agent: "BurstAgent".into(),
                user: user_id.clone()
            },
            user,
            true,
    ...
    let _us = timed!(
        STATS.login,
        UserService::login(AuthMethod::Session(Session {
            token,
            ip: "10.0.0.1".into(),
            agent: "BurstAgent".into(),
        }))
        .await
    ...
    let base = timed!(
        STATS.create_base,
        _us.create_base(format!("BurstBase_{}", worker_id))
            .await
            .map_err(|e| format!("create_base: {e}"))?
    );
    let base_id = BaseId(base.id.as_ref().ok_or("no base id")?.0.clone());
    drop(_us);

    let mut bs = BaseService::new(base_id.clone(), user_id.clone())
        .await
        .map_err(|e| format!("BaseService::new: {e}"))?;
    let table = timed!(
        STATS.create_table,
        bs.create_table("T".into())
            .await
    ...
    {
        let mut ts = TableService::new(table_id.clone(), base_id.clone(), user_id.clone())
            .await
            .map_err(|e| format!("TableService::new: {e}"))?;
        for (fname, fconfig) in field_configs() {
            timed!(
                STATS.create_field,
                ts.create_field(InsertField::new(
                    fname.to_string(),
                    fconfig,
                    fname == "Name",
                    true,
                ...
                .await
                .map_err(|e| format!("create_field({fname}): {e}"))?
            );
        }
        ...
        timed!(
            STATS.create_record,
            ts.create_a_lot_of_records(seed_records)
                .await
    ...
    loop {
        ...
        let records = match timed!(
            STATS.list_records,
            ts.list_records(PaginationParams {
                offset: Some(offset),
                limit: Some(10)
            })
            .await
        ...
        if rng.random_range(0..5) == 0 {
            let record =
                make_random_record(&table_id, &mut rng).map_err(|e| format!("make_record: {e}"))?;
            let mut ts =
                match TableService::new(table_id.clone(), base_id.clone(), user_id.clone()).await {
                    Ok(ts) => ts,
                    Err(_) => {
                        ERRORS.fetch_add(1, Ordering::Relaxed);
                        continue;
